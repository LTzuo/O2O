...
//	private final static Pattern phone_pattern = Pattern.compile("^(13|14|15|16|17|18|19)\\d{9}$");
	...
	private final static Pattern phone_pattern = Pattern.compile("0{0,1}(13[0-9]|14[0-9]|15[0-9]|166|199|198|17[0-9]|18[0-9])[0-9]{8}");
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
	...
	public static boolean isIdCard(String str){
		return REGEX_ID_CARD.matcher(str).matches();
	}
	...
	public static boolean checkPhoneNum(String phonenum){
		if(isEmpty(phonenum)){
			ToastUtil.ShortToast("请您输入手机号");
			return false;
		}
		if(!isPhone(phonenum)){
			ToastUtil.ShortToast("请您输入正确格式手机号");
			return false;
		}
		return true;
	}
	...
	public static boolean checkPwd(String pwd1,String pwd2){
		...
		if(6>pwd1.length() || pwd1.length()>15){
			ToastUtil.ShortToast("验证码长度6-15");
			return false;
		}
		if(isEmpty(pwd2)){
			ToastUtil.ShortToast("请输入密码");
		...
		if(6>pwd2.length() || pwd2.length()>15){
			ToastUtil.ShortToast("验证码长度6-15");
			return false;
		}
		if(!pwd1.equals(pwd2)){
	 ...
	 * @return
     */
	public static boolean checkBoxcheck(boolean isCheck){
		if(!isCheck){
